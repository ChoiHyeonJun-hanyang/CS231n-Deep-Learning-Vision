...
- **배운 점**
  1. 이미지를 분류하는 과정에서 여러 문제점 존재
     (배경, 다양한 종류, 그림자, 다양한 형태 등)
  2. L1 Distance에 대한 이해
     (좌표축을 따라 이동하면서 거리를 측정하기에 축이 회전한다면 거리가 달라짐)
  3. Hyperparameters의 값을 설정할 때, 여러 방식이 존재
     (Validation Set을 사용, Cross-Validation 방식을 사용 등)
  4. Softmax loss는 우리가 Linear Classifiers에서 Wx + b로 얻은 scores를 확률로 바꿔주는 장치
  5. Softmax의 Initialization 상황에서 모든 scores가 같다면, loss는 -log(1/C) = log(C)
     (추후 Sanity Check에서 사용)
  6. SVM loss는 classes의 scores에 대해서 정답 class의 scores과 얼마나 차이가 나는지를 확인
     (max(0, sj - syi + 1) 을 사용)
  7. SVM에서 W가 작아서 s가 0에 가깝다면, loss는 C - 1
- **내가 가진 의문, 의문에 대한 답변 (주로 AI 이용)**
  1. 수업 중, K-NN의 방식에선 빈 공간이 많으면 의미가 없다라는 말에 대한 의문
     -> 우리가 판단하려는 공간을 충분히 채울만큼의 다양한 sample을 확보해야함
  2. 수업 중 나온 Kullback-Leibler divergence와 Cross Entropy의 관계에 대한 의문
     -> Cross Entropy는 KL divergence를 사용한 단순한 objective function, 주로 One-hot encoding에 사용
  3. Softmax loss와 SVM loss의 개념을 비교하건대,
     SVM은 단순히 정답 scores가 다른 scores보다 margin 이상의 차이를 보여주면 loss가 늘어나지 않음
     Softmax는 loss가 아무리 정확하더라도 0이 될 수 없기때문에 Softmax가 더욱 정확한 결과를 낸다고 할 수 있는지에 관한 의문
     -> SVM은 특정 조건만 만족한다면 더 이상 그 부분의 성능 향상을 요구하지 않지만, Softmax는 아무리 classify가 성공했더라도 계속해서 loss를 발생시키기에 더욱 좋은 성능을 내게 함
