...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        self.params['W2'] = np.random.normal(loc = 0.0, scale = weight_scale, size = (num_filters * pool_H * pool_W, hidden_dim))
    ...
    def loss(self, X, y=None):
        ...
        upstream, dW3, grads['b3'] = affine_backward(dscores, affine_cache)
        dW3 += 0.5 * self.reg * W3
        grads['W3'] = dW3

        upstream, dW2, grads['b2'] = affine_relu_backward(upstream, affine_relu_cache)
        dW2 += 0.5 * self.reg * W2
        grads['W2'] = dW2

        upstream, dW1, grads['b1'] = conv_relu_pool_backward(upstream, conv_relu_pool_cache)
        dW1 += 0.5 * self.reg * W1
        grads['W1'] = dW1
