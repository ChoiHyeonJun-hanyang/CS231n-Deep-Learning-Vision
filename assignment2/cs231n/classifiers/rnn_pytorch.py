...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # Use affine transformation to compute the initial hidden state from the image features
        ...
        # Use either a vanilla RNN or LSTM (depending on self.cell_type)
        if self.cell_type == "rnn":
            hidden_state = rnn_forward(embedded, initial_hidden_state, Wx, Wh, b)
        if self.cell_type == "lstm":
            pass

        # Use a (temporal) affine transformation to compute scores
        scores_vocab = temporal_affine_forward(hidden_state, W_vocab, b_vocab)

        # Use (temporal) softmax to compute loss using captions_out
        loss = temporal_softmax_loss(scores_vocab, captions_out, mask)
    ...
    def sample(self, features, max_length=30):
        ...
        initial_hidden_state = affine_forward(features, W_proj, b_proj)
        initial_input = self._start * torch.ones((N, ), dtype = torch.long)

        inputs = []
        inputs.append(initial_input)

        hiddens = []
        hiddens.append(initial_hidden_state)

        if self.cell_type == "rnn":
            for i in range(max_length):
                prev_h = hiddens[i]
                curr_input = inputs[i]
                curr_embed = word_embedding_forward(curr_input, W_embed)
                curr_h = rnn_step_forward(curr_embed, prev_h, Wx, Wh, b)
                scores = affine_forward(curr_h, W_vocab, b_vocab)
                indexes = torch.argmax(scores, dim = 1)
                captions[:, i] = indexes
                hiddens.append(curr_h)
