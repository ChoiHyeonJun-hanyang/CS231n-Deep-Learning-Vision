...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        embed_captions = self.positional_encoding(embed_captions)
        proj_features = self.visual_projection(features).unsqueeze(1)

        tgt_mask = torch.ones((T, T), device = captions.device)
        tgt_mask = torch.tril(tgt_mask)

        output = self.transformer(embed_captions, proj_features, tgt_mask)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        patched_image = self.positional_encoding(patched_image)
        patched_image = self.transformer(patched_image)
        patched_image = torch.mean(patched_image, dim = 1)
        logits = self.head(patched_image)
