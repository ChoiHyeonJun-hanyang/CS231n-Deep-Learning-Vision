...
def get_similarity_no_loop(text_features, image_features):
    ...
    image_norm = torch.nn.functional.normalize(image_features, dim = 1)
...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    preprocessed_image = [clip_preprocess(Image.fromarray(img)).unsqueeze(0) for img in images]
    images_tensor = torch.cat(preprocessed_image, dim = 0).to(device)

    texts_tensor = clip.tokenize(class_texts).to(device)

    logits_per_image, _ = clip_model(images_tensor, texts_tensor)
    pred_arg = torch.argmax(logits_per_image, dim = 1)
    pred_classes = [class_texts[idx.item()] for idx in pred_arg]
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        processed_image = [clip_preprocess(Image.fromarray(img)).unsqueeze(0) for img in images]
        images_tensor = torch.cat(processed_image, dim = 0).to(device)
        images_features = clip_model.encode_image(images_tensor)
        self.images = images_features / images_features.norm(dim = 1, keepdim = True)
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        similarities = torch.matmul(self.images, text_features.T).squeeze()
...
class DINOSegmentation:
    def __init__(self, device, num_classes: int, inp_dim : int = 384):
        ...
        self.device = device
        self.linear = torch.nn.Linear(inp_dim, num_classes, device = self.device)
        self.num_classes = num_classes
        self.optimizer = torch.optim.Adam(self.linear.parameters(), lr = 9e-3, weight_decay = 3e-2)
