...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        exp_sim = torch.exp(sim(z_k, z_k_N) / tau)
        sum_sim_left, sum_sim_right = 0, 0

        for i in range(N):
            if i == k:
                sum_sim_left += torch.exp(sim(z_k, out[i+N]) / tau)
                sum_sim_right += torch.exp(sim(z_k_N, out[i]) / tau)
                continue
            sum_sim_left += (torch.exp(sim(z_k, out[i]) / tau) + torch.exp(sim(z_k, out[i+N]) / tau))
            sum_sim_right += (torch.exp(sim(z_k_N, out[i]) / tau) + torch.exp(sim(z_k_N, out[i+N]) / tau))
        
        loss_left = -torch.log(exp_sim / sum_sim_left)
        loss_right = -torch.log(exp_sim / sum_sim_right)
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = dot_product / (torch.linalg.norm(out_left, dim = 1, keepdim = True) * torch.linalg.norm(out_right, dim = 1, keepdim = True))
...
def compute_sim_matrix(out):
    ...
    out_linal = torch.linalg.norm(out, dim = 1, keepdim = True)
    sim_matrix = torch.matmul(out, out.transpose(0, 1)) / torch.matmul(out_linal, out_linal.transpose(0, 1))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    positive_sims = torch.exp(sim_positive_pairs(out_left, out_right) / tau)
    
    # Step 3: Compute the numerator value for all augmented samples.
    numerator = None
    numerator = torch.cat([positive_sims, positive_sims], dim = 0)    
