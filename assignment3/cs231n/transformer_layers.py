...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        first = torch.arange(0, max_len).view(-1, 1)
        second = 10000 ** (torch.arange(0, embed_dim, 2) / embed_dim)

        pe[:, :, 0::2] = torch.sin(first / second)
        pe[:, :, 1::2] = torch.cos(first / second)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        data_query = torch.reshape(self.query(query), (N, S, H, E//H))
        data_key = torch.reshape(self.key(key), (N, T, H, E//H))
        data_value = torch.reshape(self.value(value), (N, T, H, E//H))

        data_query = data_query.transpose(1, 2)
        data_key = data_key.transpose(1, 2)
        data_value = data_value.transpose(1, 2)

        similarities = torch.matmul(data_query, torch.transpose(data_key, 2, 3))
        similarities /= torch.sqrt(torch.tensor(E/H))

        if attn_mask is not None:
            similarities = similarities.masked_fill(attn_mask == 0, float('-inf'))

        attention_weights = self.attn_drop(torch.softmax(similarities, dim = -1))
        Y = torch.matmul(attention_weights, data_value).transpose(1, 2)
        Y = Y.reshape(N, S, E)
        output = self.proj(Y)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        div1, div2 = H // self.patch_size, W // self.patch_size
        temp = torch.reshape(x, (N, C, div1, H // div1, div2, W // div2))
        temp = torch.permute(temp, (0, 1, 2, 4, 3, 5))
        temp = torch.reshape(temp, (N, C, self.num_patches, self.patch_size, self.patch_size)).transpose(1, 2)

        temp = torch.reshape(temp, (N, self.num_patches, self.patch_dim))
        out = self.proj(temp)
